 ...
 */
const cordeToTeX = (f, a, x, e1 = 1e-16, e2 = 1e-16, nmax = 10) => {
  const sep = "\t"; // "&"
  const endl = "\n"; // "\\\\\n"
  const endt = "\n"; // "\\hline"
  // Commento
  let str = `% Metodo delle corde, f: ${f.toString()}, \$a$: ${a}, \$x_0\$: ${x}\n`;
  // Intestazioni
  str +=
    "$k$" +
    sep +
...
fs.writeFileSync(
  "tab_cor_sqrt_6.dat",
  cordeToTeX((x) => x * x - 6, 6, 3)
);
fs.writeFileSync(
  "tab_cor_sin.dat",
  cordeToTeX((x) => Math.sin(x), 3.2, 3.1)
);
fs.writeFileSync(
  "tab_cor_exp_mx.dat",
  cordeToTeX(
